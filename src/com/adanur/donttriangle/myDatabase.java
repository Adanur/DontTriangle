...
public class myDatabase extends SQLiteOpenHelper {
        ...
        static final String fLevelPngName="LevelPngName";
...
// OPTIONALLY PREPOPULATE THE TABLE WITH SOME VALUES   
                 ...
                 cv.put(fLevelPngName, "level1.png");
                         db.insert(tLevels, null, cv);
                 for (int i = 1; i < 18; i++) {
                	 cv.put(fLevelID, i+1);
                     cv.put(fLevelUnLocked, "false");
                     cv.put(fLevelPngName, "level"+(i+1)+"k.png");
                             db.insert(tLevels, null, cv);
				}
...
// THIS METHOD IS CALLED BY YOUR MAIN ACTIVITY TO READ A VALUE FROM THE DATABASE                 
         ...
         public String getLevelPng(int ID) {
        	// THIS METHOD IS CALLED BY YOUR MAIN ACTIVITY TO READ A VALUE FROM THE DATABASE                 
        	                 SQLiteDatabase myDB = this.getReadableDatabase();
        	                 String[] mySearch = new String[]{String.valueOf(ID)};
        	                 Cursor myCursor = myDB.rawQuery("SELECT "+ fLevelPngName +" FROM "+ tLevels +" WHERE "+ fLevelID +"=?",mySearch);
        	                 myCursor.moveToFirst();
        	                 int index = myCursor.getColumnIndex(fLevelPngName);
        	                 String myAnswer = myCursor.getString(index);
        	                 myCursor.close();
        	                 return myAnswer;
        	         }
...
// THIS METHOD IS CALLED BY YOUR MAIN ACTIVITY TO WRITE A VALUE TO THE DATABASE          
         ...
         public int changeLevelPng(int ID, String level)
         {
// THIS METHOD IS CALLED BY YOUR MAIN ACTIVITY TO WRITE A VALUE TO THE DATABASE          
                 SQLiteDatabase myDB = this.getWritableDatabase();
                 ContentValues cv = new ContentValues();
                 cv.put(fLevelPngName, level);
                 int numRowsAffected = myDB.update(tLevels, cv, fLevelID+"=?", new String []{String.valueOf(ID)});
                 return numRowsAffected;
         }
